...
mod tests {
    ...
    use datafusion_expr::{
        and, binary_expr, col, lit, logical_plan::builder::LogicalPlanBuilder, Expr, ExprSchemable,
        JoinType,
    };
    ...
    fn test_simplify_table_full_filter_in_scan() -> Result<()> {
        ...
        let expected =
            "TableScan: test projection=[a], full_filters=[Boolean(true) AS b IS NOT NULL]";
    ...
    fn to_timestamp_expr_wrong_arg() -> Result<()> {
        ...
        let expected = "Error parsing timestamp from 'I'M NOT A TIMESTAMP': error parsing date";
  ...
  TableScan: test"#;
    ...
    fn simplify_equijoin_predicate() -> Result<()> {
        ...
        let plan = LogicalPlanBuilder::from(t1)
            .join_with_expr_keys(t2, JoinType::Inner, (vec![left_key], vec![right_key]), None)?
            .build()?;
