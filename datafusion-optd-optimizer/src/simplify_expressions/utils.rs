...
pub fn delete_xor_in_complex_expr(expr: &Expr, needle: &Expr, is_left: bool) -> Expr {
    ...
    fn recursive_delete_xor_in_expr(expr: &Expr, needle: &Expr, xor_counter: &mut i32) -> Expr {
        match expr {
    ...
    match expr {
        Expr::BinaryExpr(BinaryExpr { left, op, right }) => {
            ...
            match op {
                ...
                // use not clause
                _ => Expr::Not(Box::new(Expr::BinaryExpr(BinaryExpr::new(left, op, right)))),
            }
        }
        // not (not A) ===> A
        Expr::Not(expr) => *expr,
    ...
    match expr {
        Expr::BinaryExpr(BinaryExpr { left, op, right }) => {
            match op {
                ...
                // use negative clause
                _ => Expr::Negative(Box::new(Expr::BinaryExpr(BinaryExpr::new(left, op, right)))),
            }
        }
        // ~(~A) ===> A
...
pub fn simpl_log(current_args: Vec<Expr>, info: &dyn SimplifyInfo) -> Result<Expr> {
    ...
    match number {
        Expr::Literal(value) if value == &ScalarValue::new_one(&info.get_data_type(number)?)? => {
...
pub fn simpl_power(current_args: Vec<Expr>, info: &dyn SimplifyInfo) -> Result<Expr> {
    ...
    match exponent {
        ...
        Expr::Literal(value) if value == &ScalarValue::new_one(&info.get_data_type(exponent)?)? => {
            Ok(base.clone())
        }
        Expr::ScalarFunction(ScalarFunction {
...
pub fn simpl_concat_ws(delimiter: &Expr, args: &[Expr]) -> Result<Expr> {
    match delimiter {
        Expr::Literal(ScalarValue::Utf8(delimiter) | ScalarValue::LargeUtf8(delimiter)) => {
            match delimiter {
