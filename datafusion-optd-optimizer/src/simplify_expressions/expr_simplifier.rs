...
use datafusion_expr::{
    and, expr, lit, or, BinaryExpr, BuiltinScalarFunction, Case, ColumnarValue, Expr, Like,
    Volatility,
};
...
impl<'a, S: SimplifyInfo> TreeNodeRewriter for Simplifier<'a, S> {
    ...
    fn mutate(&mut self, expr: Expr) -> Result<Expr> {
        use datafusion_expr::Operator::{
            And, BitwiseAnd, BitwiseOr, BitwiseShiftLeft, BitwiseShiftRight, BitwiseXor, Divide,
            Eq, Modulo, Multiply, NotEq, Or, RegexIMatch, RegexMatch, RegexNotIMatch,
            RegexNotMatch,
        };
        ...
        let new_expr = match expr {
            ...
            Expr::BinaryExpr(BinaryExpr {
                ...
                op: Modulo,
                right,
            }) if !info.nullable(&left)? && is_zero(&right) => match info.get_data_type(&left)? {
            ...
            Expr::IsNotNull(expr) | Expr::IsNotUnknown(expr) if !info.nullable(&expr)? => lit(true),

            // a is null/unknown --> false (if a is not nullable)
            Expr::IsNull(expr) | Expr::IsUnknown(expr) if !info.nullable(&expr)? => lit(false),
...
mod tests {
    ...
    fn test_evaluate_with_start_time(
        ...
        let execution_props = ExecutionProps::new().with_query_execution_start_time(*date_time);
    ...
    fn test_const_evaluator_scalar_functions() {
        ...
        let expr = call_fn("to_timestamp", vec![lit("2020-09-08T12:00:00+00:00")]).unwrap();
    ...
    fn test_evaluator_udfs() {
        ...
        let expected_expr = Expr::ScalarUDF(expr::ScalarUDF::new(Arc::clone(&fun), folded_args));
    ...
    fn test_simplify_modulo_by_one_non_null() {
        ...
        let expected = lit(0);
        assert_eq!(simplify(expr), expected);
        let expr = col("c2_non_null") % lit(ScalarValue::Decimal128(Some(10000000000), 31, 10));
        assert_eq!(simplify(expr), expected);
    }
    ...
    fn test_simplify_regex() {
        ...
        assert_change(
            ...
            col("c1").in_list(vec![lit("foo"), lit("bar"), lit("baz"), lit("qux")], false),
        );
        assert_change(
    ...
    fn simplify_with_guarantee(expr: Expr, guarantees: Vec<(Expr, NullableInterval)>) -> Expr {
        let schema = expr_test_schema();
        let execution_props = ExecutionProps::new();
        let simplifier =
            ExprSimplifier::new(SimplifyContext::new(&execution_props).with_schema(schema))
    ...
    fn simplify_inlist() {
        ...
        let subquery1 = scalar_subquery(Arc::new(test_table_scan_with_name("test1").unwrap()));
        let subquery2 = scalar_subquery(Arc::new(test_table_scan_with_name("test2").unwrap()));
        ...
        // c1 NOT IN (1, 2, 3, 4) OR c1 NOT IN (5, 6, 7, 8)
        let expr = in_list(col("c1"), vec![lit(1), lit(2), lit(3), lit(4)], true).or(in_list(
            col("c1"),
